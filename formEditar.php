...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['funcao'] == 'EditarColaborador')
{
    ...
    if (isset($_POST['id']) && !empty($_POST['id'])) {
        ...
        $sqlUsuario .= " WHERE id = (SELECT idusuario FROM colaboradores WHERE id = :id)";
        ...
        try {
            ...
            if (!empty($senha)) {
                $paramsUsuario[':password'] = password_hash($senha, PASSWORD_DEFAULT);
            }
            ...
            if (isset($_SESSION['user_id'])) {
                $stmtCheckSelf = $pdo->prepare("SELECT idusuario FROM colaboradores WHERE id = :id");
